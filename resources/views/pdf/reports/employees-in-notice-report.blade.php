...
<head>
    <meta charset="UTF-8">
    <title>Colaboradores em Aviso Prévio</title>
    <style>
...
<body>
    <h1>Relatório de Colaboradores em Aviso Prévio</h1>
        ...
        <tbody>
            @forelse($employees as $employee)
                <tr>
                    ...
                    <td>{{ $employee->currentContract?->registration_number ?? '-' }}</td>
                    <td>{{ optional($employee->admission_date)->format('d/m/Y') ?: '-' }}</td>
                    <td class="right">R$ {{ number_format((float) ($employee->salary ?? 0), 2, ',', '.') }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="10">Nenhum colaborador em aviso prévio encontrado.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
